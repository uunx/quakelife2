...
void CGameMovement::CheckParameters( void )
{
		 ...
		 player->GetMoveType() != MOVETYPE_OBSERVER )
	{
		...
		//uunx: removed this capping below
...
void CGameMovement::AirAccelerate( Vector& wishdir, float wishspeed, float accel )
{
	...
	accelspeed = accel * wishspeed * gpGlobals->frametime * player->m_surfaceFriction;	//uunx: friction here too
	...
	//uunx: removed this line: //then put it back
...
void CGameMovement::Accelerate( Vector& wishdir, float wishspeed, float accel )
{
	...
	accelspeed = accel * gpGlobals->frametime * wishspeed * player->m_surfaceFriction;	//uunx:friction here
...
void CGameMovement::FullWalkMove( )
{
	...
	// Not fully underwater
	{
		...
		if (player->GetGroundEntity() != NULL)
		{
			...
			Friction();		//uunx: THIS is the friction that matters
		}
...
bool CGameMovement::CheckJumpButton( void )
{
	...
	if (player->GetGroundEntity() == NULL)	//uunx: changed this to include the walljump, only works if you're not on the floor
	{
...
//uunx:end of trace
	...
	//uunx: flMul changed here
	flMul = 200.0f;

	// Acclerate upward
	// If we are ducking...
	float startz = mv->m_vecVelocity[2];
	//uunx: added if !walljump check
	if (!walljumped)
	{
		if ( (  player->m_Local.m_bDucking ) || (  player->GetFlags() & FL_DUCKING ) )
		{
	...
	if ( gpGlobals->maxClients == 1 )
	{
		...
		float flSpeedBoostPerc = ( !player->m_Local.m_bDucked ) ? 2.0f : 0.1f;
		float flSpeedAddition = fabs( mv->m_flForwardMove * flSpeedBoostPerc );
		float flMaxSpeed = mv->m_flMaxSpeed + ( mv->m_flMaxSpeed * flSpeedBoostPerc );
		float flNewSpeed = ( flSpeedAddition + mv->m_vecVelocity.Length2D() );

		//uunx: removed the limitation below, might have a bad time. edit: had a bad time
...
int CGameMovement::TryPlayerMove( Vector *pFirstDest, trace_t *pFirstTrace )
{
	...
	for (bumpcount=0 ; bumpcount < numbumps; bumpcount++)
	{
			...
			player->GetGroundEntity() == NULL )	
		{
			for ( i = 0; i < numplanes; i++ )
			{
				...
				{
					ClipVelocity( original_velocity, planes[i], new_velocity, 1.0 + sv_bounce.GetFloat() * (1 - player->m_surfaceFriction) );	//uunx: maybe remove friction here
				}
			}
	...
	//uunx: rough landing effects? bitch please
...
void CGameMovement::CheckFalling( void )
{
		 ...
		 player->m_Local.m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHOLD )
	{
		...
		//uunx: removed, makes bhopping look shit
